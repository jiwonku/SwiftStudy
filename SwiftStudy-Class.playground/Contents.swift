...
/*
 클래스 : 프로퍼티(클래스 안의 변수)와 메소드(클래스 안의 함수)를 가질 수 있는 컨테이너 타입을 정의하기 위한 용도.
...
상속 :
1. 객체지향 프로그래밍에서 클래스의 특징은 상속에 있다. 부모 클래스의 내용을 이어받는 자식 클래스는 부모 클래스의 기능과 속성을 이어 받으면서 자신(자식클래스)만을 위해 특정 기능을 추가하는 것이 가능하다.
2. 일반적인 코드 디자인 관점에서 부모 클래스는 공통된 부분을 구현하고 자식 클래스는 자식 클래스들 만의 고유한 기능을 추가로 구현한다.
3. 클래스와 구조체의 가장 큰 차이는 이 상속에 있다. 상속은 클래스에서만 가능한 개념이다.
4. 상속은 프로그램(소스)의 재사용을 쉽게 할 수 있게 해준다.
...
var someDate2 = myDate(y: 2002, m: 4, d: 14, isBC: false) // myDate.init과 같이 호출하지 않고 클래스 명으로 init의 매개변수들을 바로 전달 가능하다
...
각 자식 클래스 입장에서 print()라는 메소드가 실행된 것.
*/
...
/*
 ...
 프로퍼티 : 클래스에 정의된 변수. 일반적으로 멤버변수라고 부르고 언어에 따라 속성이라고 부르기도 한다.
          Objective-c 에서는 이를 프로퍼티라고 불렀으며 그대로 스위프트로 옮겨 왔음.
 
 인스턴스 : 클래스가 타입이라면 특정 변수에 클래스의 생성자를 이용해 데이터를 담았다면 이 변수는 클래스의 인스턴스를 가지고 있다 라고 표현한다.
          클래스는 그저 타입에 불과하며 실체화 되면 메모리 상에 올라가는데 이 메모리에 올라간 내용물을 인스턴스라고 하기도 한다.
 
 오브젝트 : 인스턴스와 의미가 동일하다. 일반적으로 객체지향 프로그래밍에서는 오브젝트라는 용어를 더 많이 사용한다.
          인스턴스는 클래스뿐만 아니라 다른것을 지칭하는 경우도 많기 때문이다.
 
 setter/getter : 일반적으로 프로퍼티는 그냥 변수처럼 사용하지만, 특별한 경우 그냥 대입하는 것이 아니라 추가연산을 하던가
                 데이터를 가공해서 넣는 등의 대입 전 특정 기능을 동작시키는 경우가 있다.
                 setter/getter는 이런 추가 기능을 의미한다.
                 대입용도로 사용하는게 setter, 참조 용도로 사용하는 게 getter
 */


/*
 다형성 : 객채지향 프로그래밍에서 다형성은 주로 오버라이딩과 오버로딩을 의미한다.
 
 오버라이딩 :
 부모 클래스에게서 상속 받은 메소드를 그대로 사용하지 않고, 자식 클래스에 맞게 변경하여 사용하는 것.
 예를 들어 위 클래스들에서
 => BaseDate의 name은 연,월,일 을 출력하고
 => myDate의 name은 BaseDate의 name에 AC or BC를 붙이고
 => yourDate의 name은 BaseDate의 name에 시,분,초 를 붙인다.
 
 메소드명, 파라미터, 리턴 값은 부모와 동일해야 한다.
 
 
 오버로딩 :
 같은 이름의 함수를 매개변수에 따라 다양하게 구현하는 것
 같은 이름의 함수라고 해도 입력 받는 매개변수의 갯수에 따라 다르게 구현하는 것이 가능.
...
/*
 ...
 속성이라는 것은 클래스에 값을 보관할 변수.
 이것은 그대로 값을 교체할 수 있기 대문에 어떤 값이 될지 모름.
 값의 입출력을 프로그래밍으로 제어할 수 있도록 하는 것이 computed property.
...
class Friend {
    ...
    var age : Int {
        ...
        set(newValue) {  //  괄호 없이 그냥 set만 쓴다면 반드시 newValue라는 키워드를 사용해야 함.
            if newValue > 0 {
                old = newValue
            }
        }
...
var you = Friend(name: "jason", age: 19)
you.printData()
you.age = 30
you.printData()
